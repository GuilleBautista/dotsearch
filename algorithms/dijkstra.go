...
func (a Dijkstra) generate_succesors(adj_matrix [][]int, path s.Path_t, last_v int, v []s.Node_t, closed []bool, open []int) []s.Path_t {
	...
	for i := range adj_matrix {
        ...
        if !closed[i] && adj_matrix[last_v][i] >= 0 && (open[i] < 0 || adj_matrix[last_v][i]+path.Cost <= open[i]) {
			...
			for ;true; {
				if succesors[low].Cost >= newpath.Cost {
					succesors = append(append(append(tmp, succesors[:low]...), newpath), succesors[low:]...)
					break
				}else if succesors[high].Cost <= newpath.Cost {
				...
				if succesors[mid].Cost > newpath.Cost {
					high = mid
				}else if succesors[mid].Cost < newpath.Cost {
					low = mid
				}else {
...
func (a Dijkstra) Solve()(s.Path_t, error){
	...
	//p_viewer := viewer.PathViewer{s.Path_t{}}
    ...
    for ; path_list.Len() > 0 ;  {
		...
		for n := path_list.Front(); n!=nil; n=n.Next() {
			...
			for ;i < len(succesors) && n.Value.(s.Path_t).Cost > succesors[i].Cost; {
